...
def createSequential(board_sz, w2v_sz, k=1000, n=4,h=200):
	modules = []
	...
	modules.append(l)
	activation = torch.nn.Softmax(dim=1) #generalization of sigmoid function - sums, then normalizes so probs sum to 1
	modules.append(activation)
...
def BasicLoss(probDistribution,boardDict,w2vDict,assassinWeight=2.0): #boardDict is word2vec Tensor version of board with labels 
	# probDistribution = torch.sum(probDistribution, dim=0) #why?
	idx,prob= torch.argmax(probDistribution),torch.max(probDistribution)
